...
float voltage = 0;
float lux;
float Apertute[] = {1, 1.4, 2, 2.8, 4, 5.6, 8, 11, 16, 22, 32, 45, 64, 90};
float iso[] = {6, 12, 25, 50, 100, 200, 400, 800, 1600, 3200, 6400, 12800, 25600, 51200};
float exposuretime;
int Apertute_num = 2, iso_num = 4;
float ev;
...
int main(void)
{
  ...
  while (1)
  {
    ...
    exposuretime = pow(Apertute[Apertute_num], 2) * 64 / (lux * iso[iso_num]);
    if (exposuretime > 60)
	{ 
	...
	if(lux<=0)tmode=0;
	ev = log(pow(Apertute[Apertute_num], 2)) / log(2) + log(1 / exposuretime) / log(2);
    usb_printf("lux=%f\n", lux);
    delay_ms(1000);
  }
}
...
__M_PAGE(UI_MENU, "Menu", PAGE_NULL,
         {
           ...
           switch(tmode)
		   {
			   case 0:SOLGUI_Widget_OptionText(3, "overload or no light");break;
			   case 1:SOLGUI_Widget_OptionText(3, "%ds", tsec);break;
			   case 2:SOLGUI_Widget_OptionText(3, "time:1/%d", tfr);break;
		   }
		   SOLGUI_Widget_OptionText(4,"EV:%f",round(ev));
		   SOLGUI_Widget_OptionText(5,"ISO:%f",iso[iso_num]);
		   SOLGUI_Widget_OptionText(6,"F%f",Apertute[Apertute_num]);
		   SOLGUI_Widget_Spin(7, "isonum:", INT8, 0, 13, &iso_num);
		   SOLGUI_Widget_Spin(8, "Anum:", INT8, 0, 13, &Apertute_num);
